...
# subparser.add_argument('--domain', type=str, nargs='?', default=None, help='Filter by domain')
# subparser.add_argument('--name', type=str, nargs='?', default=None, help='Filter by name')
# subparser.add_argument('--category', type=str, nargs='?', default=None, help='Filter by category')
...
if os.path.exists(secret):
    ...
    with open(secret, 'rb') as f:
        ...
        key, rest = Crypt.Keys.KeyIV.from_bytes(content)
...
if args.cmd == 'create':
    ...
    if args.category == 'key':
        ...
        if args.password is not None and len(args.password) > 0:
            ...
            key = Crypt.Keys.KeyIV.from_password( salt=args.salt, count=args.iterations, key_length=args.length, iv_length=args.int(length//2, password=args.password))

        else:

            key = Crypt.Keys.KeyIV.generate( salt=args.salt, count=args.iterations, key_length=args.length, iv_length=int(args.length/2))
...
if args.cmd == 'list':
    ...
    secret_list = Crypt.Secrets.SecretFactory.find_secrets(
            basedir=args.basedir,
            prefix=args.prefix
        )
    ...
    for secret_info in secret_list:
        ...
        try:
            ...
            container = Crypt.Secrets.SecretFactory.load(filename=secret_info.path, key=key)
        ...
        if container is None:
            ...
            if isinstance(container, Crypt.Secrets.Container):
...
if args.cmd == 'decrypt':
    ...
    r = Cli.Utils.CliException.from_result(message='Failed to decrypt bytes.', r=status, cmd=None, stdout=stdout, stderr=stderr)
...
if args.cmd == 'encrypt':
    ...
    r = Cli.Utils.CliException.from_result(message='Failed to encrypt bytes.', r=status, cmd=None, stdout=stdout, stderr=stderr)
...
if args.cmd == 'create':
    ...
    if args.category == 'cookie':
        ...
        cookie = Crypt.Secrets.Cookie.from_key(key=key, domain=args.domain, name=args.name, category='cookie', header=header, value=value) 
        print(cookie)
        filename = Crypt.Secrets.SecretFactory.save(secret=cookie, basedir=basedir, prefix=prefix)  
    ...
    if args.category == 'login':
        ...
        login = Crypt.Secrets.Login.from_key(key=key, domain=args.domain, name=args.name, category='login', user=user, password=password) 
        print(login)
        filename = Crypt.Secrets.SecretFactory.save(secret=login, basedir=basedir, prefix=prefix)  
    ...
    if args.category == 'token':
        ...
        header = args.header.encode()
        value = args.value.encode()
        token = Crypt.Secrets.Token.from_key(key=key, domain=args.domain, name=args.name, category='token', header=header, value=value) 
        print(token)
        filename = Crypt.Secrets.SecretFactory.save(secret=token, basedir=basedir, prefix=prefix, overwrite=args.overwrite)  
